...
#define G15V2_KEYMAP_SIZE (G15V2_KEYS*3)
 ...
 */
static const unsigned int g15v2_default_key_map[G15V2_KEYS] = {
	KEY_F1,
	KEY_F2,
	KEY_F3,
	KEY_F4,
...
static void g15v2_msg_send(struct hid_device *hdev, u8 msg, u8 value1, u8 value2)
{
...
static void g15v2_led_set(struct led_classdev *led_cdev,
                        ...
                        int led_num)
{
	...
	g15v2_msg_send(hdev, 0x04, ~(g15data->led), 0);
}

static void g15v2_led_m1_brightness_set(struct led_classdev *led_cdev,
                                      enum led_brightness value)
{
	g15v2_led_set(led_cdev, value, G15V2_LED_M1);
}

static void g15v2_led_m2_brightness_set(struct led_classdev *led_cdev,
                                      enum led_brightness value)
{
	g15v2_led_set(led_cdev, value, G15V2_LED_M2);
}

static void g15v2_led_m3_brightness_set(struct led_classdev *led_cdev,
                                      enum led_brightness value)
{
	g15v2_led_set(led_cdev, value, G15V2_LED_M3);
}

static void g15v2_led_mr_brightness_set(struct led_classdev *led_cdev,
                                      enum led_brightness value)
{
	g15v2_led_set(led_cdev, value, G15V2_LED_MR);
}

static enum led_brightness g15v2_led_brightness_get(struct led_classdev *led_cdev)
{
	struct device *dev;
	struct hid_device *hdev;
	struct g15v2_data *g15data;
	...
	if (led_cdev == g15data->led_cdev[G15V2_LED_M1])
		value = g15data->led & 0x01;
	else if (led_cdev == g15data->led_cdev[G15V2_LED_M2])
		value = g15data->led & 0x02;
	else if (led_cdev == g15data->led_cdev[G15V2_LED_M3])
		value = g15data->led & 0x04;
	else if (led_cdev == g15data->led_cdev[G15V2_LED_MR])
		value = g15data->led & 0x08;
	else
		dev_info(dev, G15V2_NAME " error retrieving LED brightness\n");
                           ...
                           enum led_brightness value)
{
	...
	if (led_cdev == g15data->led_cdev[G15V2_LED_BL_KEYS]) {
		if (value > 2)
			value = 2;
		g15data->keys_bl = value;
		g15v2_msg_send(hdev, 0x01, g15data->keys_bl, 0);
	} else if (led_cdev == g15data->led_cdev[G15V2_LED_BL_SCREEN]) {
		if (value > 2)
			value = 2;
		g15data->screen_bl = value<<4;
		g15v2_msg_send(hdev, 0x02, g15data->screen_bl, 0);
	} else if (led_cdev == g15data->led_cdev[G15V2_LED_BL_CONTRAST]) {
		if (value > 63)
			value = 63;
		g15data->screen_contrast = value;
		g15v2_msg_send(hdev, 32, 129, g15data->screen_contrast);
	} else
		dev_info(dev, G15V2_NAME " error retrieving LED brightness\n");
...
static const struct led_classdev g15v2_led_cdevs[7] = {
	{
		.brightness_set		= g15v2_led_m1_brightness_set,
		.brightness_get		= g15v2_led_brightness_get,
	},
	{
		.brightness_set		= g15v2_led_m2_brightness_set,
		.brightness_get		= g15v2_led_brightness_get,
	},
	{
		.brightness_set		= g15v2_led_m3_brightness_set,
		.brightness_get		= g15v2_led_brightness_get,
	},
	{
		.brightness_set		= g15v2_led_mr_brightness_set,
		.brightness_get		= g15v2_led_brightness_get,
	},
	{
...
static void g15v2_notify_keymap_switched(struct gcommon_data * gdata,
                                       unsigned int index)
{
	struct g15v2_data * g15data = gdata->data;

	g15data->led = 1 << index;
	g15v2_msg_send(gdata->hdev, 4, ~g15data->led, 0);
}

static ssize_t g15v2_name_show(struct device *dev,
...
static ssize_t g15v2_name_store(struct device *dev,
                              struct device_attribute *attr,
 ...
 * The "minor" attribute
 */
static ssize_t g15v2_minor_show(struct device *dev,
                              struct device_attribute *attr,
                              char *buf)
{
	struct gcommon_data *gdata = dev_get_drvdata(dev);
...
static DEVICE_ATTR(minor, 0444, g15v2_minor_show, NULL);
...
static void g15v2_initialize_keymap(struct gcommon_data *gdata)
{
	int i;

	for (i = 0; i < G15V2_KEYS; i++) {
		gdata->input_data.keycode[i] = g15v2_default_key_map[i];
                     ...
                     const struct hid_device_id *id)
{
	...
	dbg_hid("Preparing to parse " G15V2_NAME " hid reports\n");

	/* Parse the device reports and start it up */
	error = hid_parse(hdev);
	if (error) {
                dev_err(&hdev->dev, G15V2_NAME " device report parse failed\n");
		error = -EINVAL;
	...
	if (error) {
		dev_err(&hdev->dev, G15V2_NAME " hardware start failed\n");
		error = -EINVAL;
		goto err_cleanup_g15data;
	}

	dbg_hid(G15V2_NAME " claimed: %d\n", hdev->claimed);

	error = hdev->ll_driver->open(hdev);
	if (error) {
		dev_err(&hdev->dev, G15V2_NAME " failed to open input interrupt pipe for key and joystick events\n");
		error = -EINVAL;
	...
	if (gdata->input_dev == NULL) {
		dev_err(&hdev->dev, G15V2_NAME " error initializing the input device");
		error = -ENOMEM;
		goto err_cleanup_hw_start;
	}
	...
	gdata->input_dev->name = G15V2_NAME;
	...
	gdata->input_dev->keycodemax = G15V2_KEYMAP_SIZE;
	...
	gdata->input_data.notify_keymap_switched = g15v2_notify_keymap_switched;

	error = ginput_alloc(gdata, G15V2_KEYS);
	if (error) {
		dev_err(&hdev->dev, G15V2_NAME " error allocating memory for the input device");
		goto err_cleanup_input_dev;
	}

	g15v2_initialize_keymap(gdata);

	error = input_register_device(gdata->input_dev);
	if (error) {
		dev_err(&hdev->dev, G15V2_NAME " error registering the input device");
		error = -EINVAL;
	...
	list_for_each_entry(report, feature_report_list, list) {
		...
		dbg_hid(G15V2_NAME " Feature report: id=%u type=%u size=%u maxfield=%u report_count=%u\n",
	...
	list_for_each_entry(report, output_report_list, list) {
		dbg_hid(G15V2_NAME " output report %d found size=%u maxfield=%u\n", report->id, report->size, report->maxfield);
		if (report->maxfield > 0) {
			dbg_hid(G15V2_NAME " offset=%u size=%u count=%u type=%u\n",
	...
	for (i = 0; i < 7; i++) {
		...
		if (g15data->led_cdev[i] == NULL) {
			dev_err(&hdev->dev, G15V2_NAME " error allocating memory for led %d", i);
			error = -ENOMEM;
			goto err_cleanup_led_structs;
		}
		/* Set the accessor functions by copying from template*/
		*(g15data->led_cdev[i]) = g15v2_led_cdevs[i];
		...
		if (led_name == NULL) {
			dev_err(&hdev->dev, G15V2_NAME " error allocating memory for led %d name", i);
			error = -ENOMEM;
			goto err_cleanup_led_structs;
		}
		switch (i) {
	...
	for (i = 0; i < 7; i++) {
		...
		if (error < 0) {
			dev_err(&hdev->dev, G15V2_NAME " error registering led %d\n", i);
			error = -EINVAL;
	...
	spin_lock_irqsave(&gdata->lock, irq_flags);
	if (g15data->ready_stages != G15V2_READY_STAGE_1) {
		dev_warn(&hdev->dev, G15V2_NAME " hasn't completed stage 1 yet, forging ahead with initialization\n");
		/* Force the stage */
	...
	if (g15data->ready_stages != G15V2_READY_STAGE_2) {
		dev_warn(&hdev->dev, G15V2_NAME " hasn't completed stage 2 yet, forging ahead with initialization\n");
		/* Force the stage */
		g15data->ready_stages = G15V2_READY_STAGE_2;
	}
	init_completion(&g15data->ready);
	 ...
	 * Clear the LEDs
	 */
	g15v2_msg_send(hdev, 4, ~g15data->led, 0);
...
static void g15v2_post_reset_start(struct hid_device *hdev)
{
	unsigned long irq_flags;
