...
static enum led_brightness g19_led_mbtns_brightness_get(struct led_classdev *led_cdev)
{
	struct hid_device *hdev = gcore_led_classdev_to_hdev(led_cdev);
	struct gcore_data *gdata = hid_get_gdata(hdev);
	struct g19_data *g19data = gdata->data;
...
static void g19_led_bl_send(struct hid_device *hdev)
{
	...
	g19data->backlight_report->field[0]->value[0] = g19data->backlight_rgb[0];
	g19data->backlight_report->field[0]->value[1] = g19data->backlight_rgb[1];
	g19data->backlight_report->field[0]->value[2] = g19data->backlight_rgb[2];
...
static void g19_ep1_urb_completion(struct urb *urb)
{
	...
	if (likely(urb->status == 0)) {
		...
		for (i = 0; i < 8; i++)
			gcore_input_report_key(gdata, 24+i, g19data->ep1keys[0]&(1<<i));
...
static int read_feature_reports(struct gcore_data *gdata)
{
	...
	if (list_empty(feature_report_list)) {
		dev_err(&hdev->dev, "%s no feature report found\n", gdata->name);
		return -ENODEV;
	}
...
static void wait_ready(struct gcore_data *gdata)
{
	...
	spin_lock_irqsave(&gdata->lock, irq_flags);
	if (g19data->ready_stages != G19_READY_STAGE_1) {
		dev_warn(&hdev->dev, "%s hasn't completed stage 1 yet, forging ahead with initialization\n", gdata->name);
		/* Force the stage */
	...
	if (g19data->ready_stages != G19_READY_STAGE_2) {
		dev_warn(&hdev->dev, "%s hasn't completed stage 2 yet, forging ahead with initialization\n", gdata->name);
		/* Force the stage */
		g19data->ready_stages = G19_READY_STAGE_2;
	}
	init_completion(&g19data->ready);
...
static void send_finalize_report(struct gcore_data *gdata) 
{
	...
	if (g19data->ready_stages != G19_READY_STAGE_3) {
		dev_warn(&hdev->dev, "%s hasn't completed stage 3 yet, forging ahead with initialization\n", gdata->name);
		/* Force the stage */
		g19data->ready_stages = G19_READY_STAGE_3;
	} else {
...
static int g19_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if (gdata == NULL) {
		dev_err(&hdev->dev, G19_NAME " can't allocate space for device attributes\n");
		error = -ENOMEM;
		goto err_no_cleanup;
	}
	...
	error = gcore_input_probe(gdata, g19_default_keymap, ARRAY_SIZE(g19_default_keymap));
	if (error) {
	...
	error = gcore_leds_probe(gdata, g19_led_cdevs, ARRAY_SIZE(g19_led_cdevs));
	if (error) {
	...
	if (gdata->gfb_data == NULL) {
		dev_err(&hdev->dev, "%s error registering framebuffer\n", gdata->name);
		goto err_cleanup_leds;
	}

	error = sysfs_create_group(&(hdev->dev.kobj), &g19_attr_group);
	if (error) {
		dev_err(&hdev->dev, "%s failed to create sysfs group attributes\n", gdata->name);
